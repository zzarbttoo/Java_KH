...
for (var j = 1; j < 15; j++) {
	...
	var queryParams = '?'
			...
			+ '=Cs%2B%2FttyigCPsJ%2FJlwnLzps%2B2XP1Ol4EQmpZ2Jzt5S0B%2Fne4ZOYKUem1dtcO%2BOyBKXoouQTWwvxgl2rwpxf9xvA%3D%3D'; /*
																															 * Service
																															 * Key
																															 */
	// 페이지는 14(693)이 max
...
function parseApi() {
	request(
			...
			function(error, response, body) {
				...
				xmlTojson = convert.xml2json(body, {
					compact : true,
					spaces : 4
				});
				...
				for (var i = 0; i < jsonToObject.response.body.items.item.length; i++) {
					...
					whereleave = jsonToObject.response.body.items.item[i].ctprvn_nm._text
							+ ' '
							+ jsonToObject.response.body.items.item[i].sigungu_nm._text
							+ ' '
							+ jsonToObject.response.body.items.item[i].emd_nm._text;
					...
					if (jsonToObject.response.body.items.item[i].lvstck_nm._text == '소') {
						...
						whereleave = whereleave.replace(/undefined/gi, '');
