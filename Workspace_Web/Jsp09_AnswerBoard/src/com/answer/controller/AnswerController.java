...
public class AnswerController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			} else if (command.equals("subinsert")) {

				int boardno = Integer.parseInt(request.getParameter("boardno"));

				System.out.println("디버깅");
				// updateanswer 선택한 글 groupseq, boardno
				// insert answer boardno, boardno, boardno, title, content, writer

				AnswerDto parentdto = biz.selectOne(boardno);
				System.out.println("parentdt num");
				System.out.println(parentdto.getBoardno());

				request.setAttribute("dto", parentdto);
				dispatch("subinsert.jsp", request, response);

			} else if (command.equals("subinsertres")) {

				int parentboardno = Integer.parseInt(request.getParameter("boardno"));
				int parentGseq = Integer.parseInt(request.getParameter("groupseq"));
				String title = request.getParameter("title");
				String writer = request.getParameter("writer");
				String content = request.getParameter("content");

				AnswerDto insertDto = new AnswerDto(parentboardno, parentGseq, title, writer, content);

				int res = biz.answerProc(insertDto);
